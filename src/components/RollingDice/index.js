...
export function RollingDice() {
  ...
  useEffect(() => {
    ...
    if (payload.type?.startsWith("attack")) {
      ...
      setRenderBoxes(
        <>
          <BoxResult
            title="ATAQUE"
            total={rolled.attack.total}
            totalColor={rolled.attack.totalColor}
            rows={rolled.attack.infos}
          />
          <BoxResult
            title="DANO"
            total={rolled.damage.total}
            rows={rolled.damage.infos}
          />
        </>
      );
    }
    if (payload.type === "skill") {
      ...
      setRenderBoxes(
        <BoxResult
          title={rolled.title.toUpperCase()}
          total={rolled.total}
          totalColor={rolled.totalColor}
          rows={rolled.infos}
        />
      );
    }
  ...
  return (
    <Modal
      ...
      visible={rolling}
      onRequestClose={() =>
        dispatch(setRolling({ rolling: false, payload: "" }))
      }
    >
